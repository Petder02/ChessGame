...
//TODO: Implement a Swing Display for Xiangqi
/**
...
public class SwingChessBoard implements ChessBoard
{
  ...
  private final int numRows;                                 // number of rows of the board given the game type
  private final int numColumns;                              // number of rows of the board given the game type
    ...
    try {
      SwingUtilities.invokeAndWait(new Runnable() {
        public void run() {
          ...
          //JPanel panel creates a grid for the squares and the listener for the user clicks
          JPanel panel = new JPanel(new GridLayout(numRows, numColumns));
          //ActionListner responder stores a ChessAction which responds to button clicks
          ActionListener responder = new ChessAction();
          
          //Loop-Goal: create the squares, each with a responder
  ...
  public void addPiece(final ChessPiece piece, final int row, final int col) {
    ...
    //Runnable addPiece stores a thread which runs the display code on the event dispatch thread
  ...
  public ChessPiece removePiece(final int row, final int col) {
    ...
    //ChessPiece save is the piece that will be removed from the board
    ChessPiece save = pieces[row][col];
    pieces[row][col] = null;
    
    //Runnable removePiece is a thread which runs the display code on the event dispatch thread
  ...
  private class ChessAction implements ActionListener {
    ...
    private void processSecondSelection(int row, int col) {
      ...
      //boolean moveMade stores whether or not a move should be made on the board
  ...
  /**
   * Starts/launches a playable game of Indo-European Chess using the Java Swing graphics
   * library for the board display
   * @param args the command line arguments
   */
  public static void main(String[] args)
  {
    //ChessGameLauncher is the class which launched the game of chess in Project 3
    //It was edited so that it does not represent a standalone application anymore
    //but instead contains one method which starts a game of EuropeanChess using the Java Swing graphics library
    ChessGameLauncher.launchSwingChessGame();
  }
